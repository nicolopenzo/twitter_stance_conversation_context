...
from dataset_conversations_savings import TweetConversationsBERT, create_data_loaderBERT
import torch
from model_conversations import TweetConversationBERT
...
train_loader = create_data_loaderBERT(train_posts, train_users, train_chains, train_fin_trees, train_fin_parents,
                                  train_graph_source, train_graph_dest, train_graph_time, train_discrete_graphs,
                                  tokenizer, MAX_LEN_BERT,  train_labels, BATCH_SIZE)

dev_loader = create_data_loaderBERT(train_posts, train_users, train_chains, train_fin_trees, train_fin_parents,
                                  train_graph_source, train_graph_dest, train_graph_time, train_discrete_graphs,
                                  tokenizer, MAX_LEN_BERT,  dev_labels, BATCH_SIZE)


test_loader = create_data_loaderBERT(test_posts, test_users, test_chains, test_fin_trees, test_fin_parents,
                                  test_graph_source, test_graph_dest, test_graph_time, test_discrete_graphs,
                                  tokenizer, MAX_LEN_BERT,  test_labels, BATCH_SIZE, shuffle=False)
...
# create the model
model = TweetConversationBERT(4, device).to(device)
